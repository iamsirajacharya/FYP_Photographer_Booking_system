...
exports.login = async (req, res) => {
  try {
    console.time("Login Time"); // Start timing the login process

    const { email, password } = req.body;

    if (!email || !password) {
      return new responseError(res, 400, "All fields are required");
    }
    ...
    if (!user) {
      ...
      return new responseError(res, 401, "Invalid email or password");
    }
    ...
    if (!isValidPassword) {
      console.timeEnd("Login Time");
      return new responseError(res, 401, "Wrong password");
    }
    ...
    new responseError(res, 500, "Internal server error");
  }
};
