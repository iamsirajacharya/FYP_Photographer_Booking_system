...
const fieldValidation = (field, next) => {
  if (!field) {
    return next(new responseError(`Missing fields`, 400));
  }
};
...
exports.register = handleAsync(async (req, res, next) => {
  try {
    const { username, email, password, phone_number, speciality, photo } =
      req.body;
    ...
    fieldValidation(phone_number, next);
    fieldValidation(speciality, next);
    fieldValidation(photo, next);
