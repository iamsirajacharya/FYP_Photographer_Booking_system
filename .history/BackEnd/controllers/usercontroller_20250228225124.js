...
exports.register = handleAsync(async (req, res) => {
  try {
    ...
    // Map speciality to role enum value
    let role;
    if (speciality.toLowerCase() === "client") {
      role = "client";
    } else if (speciality.toLowerCase() === "photographer") {
      role = "photographer";
    } else {
    ...
    // Create user - note we don't need to hash password here as the model hook will do it
    const newUser = await User.create({
      Username,
      email,
      password,
      phone, // Changed from phone_number to match model
      role, // Using the mapped role from speciality
      profile_image,
      // location is optional, so not included unless provided
      location: req.body.location || null,
    });

    // Response - Don't include password in response
    const userResponse = {
      id: newUser.id,
