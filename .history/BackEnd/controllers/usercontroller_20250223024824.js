...
const fieldValidation = (field, fieldName) => {
  if (!field) {
    console.log(`Validation failed for: ${fieldName}`); // Log validation failures
...
exports.register = handleAsync(async (req, res) => {
  ...
  console.log("Request Body:", req.body); // Debugging to see what the request body looks like

  const profilePhoto = req.file ? req.file.buffer : null;

  // Log each field before validation
