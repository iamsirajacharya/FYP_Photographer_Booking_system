...
db.sequelize
  .sync({ force: true })
  .then(async () => {
    // 1) Optionally read from environment variables
    const adminName = process.env.ADMIN_NAME;
    const adminEmail = process.env.ADMIN_EMAIL;
    const adminPassword = process.env.ADMIN_PASSWORD;

    // 2) Check if an admin user with this email already exists
    ...
    if (!existingAdmin) {
      ...
      await db.User.create({
        name: adminName,
        email: adminEmail,
        password: adminPassword, // Will be hashed automatically via your model's hooks
        role: "admin",
      });
