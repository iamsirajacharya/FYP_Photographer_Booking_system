...
exports.getPendingApplications = catchAsync(async (req, res) => {
  ...
  // 1) Query only the fields that exist on the Photographer table
  //    and include the associated User with the correct alias "user"
  const { count, rows } = await db.Photographer.findAndCountAll({
    where: { applicationStatus: "pending" },
    attributes: [
      "id",
      "specialty",
      ...
      "equipment",
      "hourlyRate",
      "portfolioImages", // or whatever column stores the array of portfolio filenames
    ],
    include: [
      {
        model: db.User,
        as: "users", // Make sure your Photographer model uses `Photographer.belongsTo(User, { as: "user", ... })`
  ...
  // 2) Map each Photographer row into a plain object for the frontend
  const applications = rows.map((photographer) => ({
    id: photographer.id,
    ...
    appliedDate: photographer.appliedDate,
    location: photographer.location,
    equipment: photographer.equipment,
    hourlyRate: photographer.hourlyRate,
    portfolio: photographer.portfolioImages, // rename as needed
  }));
