...
const MessageNotification = () => {
  ...
  // Handle notification click to prepare data for the messages page
  const handleNotificationClick = (notification) => {
    setIsOpen(false);

    // Store the conversation data in localStorage for persistence
    if (user?.role === "photographer") {
      ...
      const clientInfo = {
        ...
        image:
          notification.profileImage || "/placeholder.svg?height=64&width=64",
      ...
      // Pre-fetch and store messages if possible
      fetchAndStoreMessages(notification.id);
    } else {
      // For clients
      ...
      localStorage.setItem(
        "activeConversation",
        JSON.stringify(photographerInfo)
      );
  ...
  // Fetch and store messages for the selected conversation
  const fetchAndStoreMessages = async (conversationId) => {
    try {
      const response = await fetch(
      ...
      if (response.ok) {
        ...
        // Store messages based on user role
        if (user?.role === "photographer") {
          localStorage.setItem(
            `photographer_messages_${conversationId}`,
