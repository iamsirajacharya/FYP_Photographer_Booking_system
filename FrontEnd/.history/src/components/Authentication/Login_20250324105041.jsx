  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const body =
        activeTab === "login"
          ...
          : {
              ...
              // Optional: you might want to confirm server-side that
      ...
      const response = await fetch(endpoint, {
        ...
        credentials: "include", // Important to allow backend cookies
      ...
      // data.user and data.message exist according to your backend
      // The token is in an HTTP-only cookie, not in data.token
      const { user } = data;

      // Dispatch to set user in Redux; token is stored in a cookie,
      // so we can pass null or an empty string for token
      dispatch(
        setCredentials({
          user,
          token: null, // The server sets it in an HTTP-only cookie
          role: user.role,
        })
      );
