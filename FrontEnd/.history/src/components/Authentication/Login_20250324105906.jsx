...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // data.user is returned by your backend
      const { user } = data;

      // Dispatch to set user in Redux; the token is in an HTTP-only cookie
      dispatch(
        setCredentials({
          user,
      ...
      if (activeTab === "login") {
        ...
        // 3) Navigate to /user/dashboard after successful login
        ...
        // You could also navigate somewhere else after registration
        // navigate("/user/dashboard");
      }
    } catch (error) {
